...
public class PixelMapPlus extends PixelMap implements ImageOperations 
{
	...
	public void zoomIn(int x, int y, double zoomFactor) throws IllegalArgumentException
	{
		...
		int leftX = (int)((double)x - (double)width / zoomFactor / 2);
		int rightX = (int)((double)x + (double)width / zoomFactor / 2);
		int topY = (int)((double)y + (double)height / zoomFactor / 2);
		int botY = (int)((double)y - (double)height / zoomFactor / 2);
		
		leftX = leftX < 0 ? 0 : leftX;
		rightX = rightX > width ? width-1 : rightX;
		topY = topY > height ? height-1 : topY;
		botY = botY < 0 ? 0 : botY;
		
		System.out.print(botY);
		AbstractPixel[][] dataCpy = new AbstractPixel[topY-botY][rightX-leftX];
